...
message.channel.send(`**:gift: Önce bir ödülümüzü belirleyelim. Yaz bakalım ödül ne olmasını istiyorsun?**`).then(() => {
...
if(!ch) return message.channel.send(`Etiketlediğin isimde kanal yok bu yüzden işlem iptal edildi.`)
message.delete()
message.channel.send(`Kanal: ${ch}`).then(() => kanal.push(ch.id))
  
message.channel.send(`**Çekiliş ne zamana kadar sürecek? (Örn: 1 dakika/1 saat)**`).then(() => {
message.channel.awaitMessages(filter, { maxMatches: 1, time: 60000, errors: ['time'] }).then(collected => {
...
message.channel.send(`Süre: ${az}`).then(() => zaman.push(collected.first().content)).then(()=> {
  ...
  .setTimestamp()
  .setFooter(`asteroid | Çekiliş Vakti!`)
